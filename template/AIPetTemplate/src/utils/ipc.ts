...
export const setOrientation = (dir = 2) => {
  ...
  // 提前变更状态，让横屏下需隐藏的组件隐藏
  if (dir === 2) {
    dispatch(updateIpcCommon({ isFull: true }));
  } else {
    dispatch(updateIpcCommon({ fullPlayer: false }));
  }
    ...
    setPageOrientation({
      ...
      success: () => {
        if (dir === 2) {
          ...
          dispatch(updateIpcCommon({ fullPlayer: true, showFullButton: true }));
        } else {
          setNavigationBarBack({
            type: 'system',
          });
          showMenuButton();
          showStatusBar();
          dispatch(updateIpcCommon({ isFull: false }));
        }
      },
    });
  }, 0);
