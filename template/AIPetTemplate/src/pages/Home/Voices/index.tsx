...
import { authorizeRecord, getCdnPath } from '@/utils';
...
const Voices: FC<Props> = ({ ready, onClose }) => {
  ...
  const [addPopupReady, setAddPopupReady] = useState(false);
    ...
    <View className={styles.container}>
      ...
      {audiosTotal === 0 ? (
        <View className={styles.empty}>
          <Image src={getCdnPath('voicePlaceholder.png')} className={styles.img} />
        </View>
      ) : (
      ...
      <Popup
        ...
        onAfterEnter={() => setAddPopupReady(true)}
      >
        {showAddPopup && addPopupReady && <AddVoice onHide={() => setShowAddPopup(false)} />}
      </Popup>
