...
const DatetimePicker: React.FC<DateTimePickerProps> = props => {
  ...
  const originColumns = useMemo(() => {
    const { maxYear, maxDate: maxDay, maxMonth, maxHour, maxMinute } = getBoundary(
      'max',
      currentDateRef.current
    );
    const { minYear, minDate: minDay, minMonth, minHour, minMinute } = getBoundary(
      'min',
      currentDateRef.current
    );
  ...
  const maskBgImg = useMemo(() => {
    try {
      const { r: sR, g: sG, b: sB, a: sA } = (Color(bgColor)
        .alpha(0.95)
        .rgb() as unknown) as ColorRgba;
      const { r: eR, g: eG, b: eB, a: eA } = (Color(bgColor)
        .alpha(0.6)
        .rgb() as unknown) as ColorRgba;
