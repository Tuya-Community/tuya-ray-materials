    ...
    <>
      {editable ? (
        <View className={styles.randomPassword}>
          <Text className={styles.randomPasswordTitle}>
            {getCustomPwdTitle(digitalBase, false)}
          </Text>
          <CustomKeyboard
            inputContainerStyle={{
              marginTop: '16rpx',
            ...
            renderButton={() => (
              <View onClick={() => _getRandomPassword()} style={{ marginRight: '8rpx' }}>
                <Text style={{ color: themeColor }}>
                  {Strings.getLang('Password_getPassword_random')}
                </Text>
              </View>
            )}
