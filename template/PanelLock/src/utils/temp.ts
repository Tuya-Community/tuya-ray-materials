...
export const formatRecordItemText = ({
  ...
  if ((pwdType === 0 && scheduleList) || pwdType === 1 || pwdType === 4 || pwdType === 9) {
    ...
    const scheduleText = schedule.timeZoneId
      ? formatScheduleText(schedule.workingDay, {
  ...
  if (pwdType === 2 && isInvalid) {
    return `${Strings.getLang('Password_hasBeen')} ${formatDate(effectiveTime)} ${Strings.getLang(
      'Password_invalid'
    )}`;
  }

  return isInvalid
    ? `${Strings.getLang('Password_hasBeen')} ${formatDate(invalidTime)} ${Strings.getLang(
        'Password_invalid'
      )}`
    : `${Strings.getLang('Password_willBeen')} ${formatDate(invalidTime)} ${Strings.getLang(
        'Password_invalid'
      )}`;
};
...
export const getPwdLengthRange = (digitalBase: number, isWifi: boolean): [number, number] => {
  const minHexMap: Record<number, number> = {
    4: 8,
    5: 8,
    6: 8,
    7: 8,
  ...
  const maxHexMap: Record<number, number> = {
    4: 12,
    5: 12,
    6: 12,
    7: 11,
