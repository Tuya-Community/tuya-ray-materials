...
const Index: FC = () => {
  ...
  const { dpUnlockDynamic, dpUnlockOfflinePd, dpTemporaryPasswordCreat } = useDpPermission();
  ...
  const [currentTab, setCurrentTab] = useState<TabItemKeys>(
    compact([
  ...
  return (
    <>
      <NavBar
        title={Strings.getLang('title_temp')}
        fixed
        placeholder
        leftArrow
      ...
      <ScrollView refresherTriggered={false} scrollY className={styles.warp}>
        <View className={styles.header}>
          <View className={styles.passswordTypeWrap}>
            <Text className={styles.passwordType}>{Strings.getLang('Password_textType')}</Text>
            <Text className={styles.passwordTip}>{passwordTypeTips[currentTab]}</Text>
          </View>
              ...
              return (
                <View key={key} onClick={() => setCurrentTab(key)}>
                  <View className={clsx(styles.tabItem, isActive && styles.activeTab)}>
                    <Text style={{ color: isActive ? themeColor : '#000' }}>
                      {formTypeTitle[key]}
                    </Text>
                  </View>
                </View>
        ...
        <View className={styles.formContainer}>{tabContents[currentTab]}</View>

        <View
          className={styles.circleButton}
          style={{ backgroundColor: themeColor }}
          onClick={() => router.push('/temp-record')}
        >
