...
const Result: React.FC<IProps> = props => {
  const { dpCode, isSuccess, errText, name, setName, save } = props;
  ...
  return (
    <View className={styles.container}>
      ...
      <Text className={styles.text}>{`${Strings.getDpLang(dpCode)}${Strings.getLang(
        'addStepSuccess'
      )}`}</Text>
      ...
      <View className={styles.btnWarp}>
        <View onClick={() => save()} className={styles.btn} style={{ backgroundColor: themeColor }}>
