...
import { View, ScrollView, showModal, showLoading, hideLoading, router } from '@ray-js/ray';
...
const InValidRecord: React.FC = () => {
  ...
  const renderListItem = (item: IBleRecordItem) => {
    return <RecordItem key={item.id} isInvalid reloadList={run} {...item} />;
  };
      ...
      <View
        ...
        onClick={() => {
          showModal({
            ...
            success: res => {
              if (res.confirm) {
                Api.apis.clearInvalidPwdList().then(() => {
                  run();
                });
              }
            },
  ...
  return (
    <>
      <NavBar
        title={Strings.getLang('title_temp_invalid_record')}
        fixed
        placeholder
        leftArrow
