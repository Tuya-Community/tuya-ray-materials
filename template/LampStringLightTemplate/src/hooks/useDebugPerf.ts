 ...
 */
export const useDebugPerf = (component, props = {}, maxCount = 6, time = 500) => {
    ...
    if (count.current >= maxCount) {
      if (prevProps.current) {
        ...
        if (Object.keys(changedProps).length) {
          ...
          console.log(
            `%c[useDebugPerf =>] %c warning: render more than ${maxCount} times in ${timeStr}s; %c component name: ${componentName}`,
            'color: #75fbfd;',
            'color: #ef906e;',
            'color: #33b9ff;',
