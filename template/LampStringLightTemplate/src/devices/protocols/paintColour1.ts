...
const { paint_colour_1 } = lampSchemaMap;
...
export default class SmearFormater {
  ...
  constructor(uuid = paint_colour_1?.code, defaultValue = _defaultValue) {
    this.uuid = uuid;
    this.defaultValue = defaultValue;
  }

  parser(val = ''): SmearDataType {
    // console.log('SmearFormater-parser', val);
    if (!val || typeof val !== 'string') {
      console.warn(this.uuid, 'dp data is invalid, cannot be parsed', val);
    ...
    if ([DimmerMode.colour, DimmerMode.colourCard, DimmerMode.white].includes(dimmerMode)) {
      if (dimmerMode === DimmerMode.white) {
        ...
        if (result.smearMode !== SmearMode.all) {
          toN(step(4).value);
        }
      } else {
        // 彩光/色卡
      ...
      if ([SmearMode.clear, SmearMode.single].includes(result.smearMode)) {
    ...
    log(result, 'paint_colour_1 parser result');
    return result;
  }
  ...
  formatter(_data: SmearDataType): string {
    ...
    if ([DimmerMode.colour, DimmerMode.colourCard, DimmerMode.white].includes(dimmerMode)) {
      if (dimmerMode === DimmerMode.white) {
        if (smearMode === SmearMode.all) {
          const newResult = `${result}${nToHS(smearMode)}${nToHS(brightness, 4)}${nToHS(
            temperature,
            4
          )}`;
          return newResult;
        }
        result += `${nToHS(smearMode)}${nToHS(brightness, 4)}${nToHS(temperature, 4)}0000`;
      } else {
        // 彩光/色卡
    ...
    log(new SmearFormater().parser(result), 'paint_colour_1 formatter result');
    return result;
  }
}
