...
export default class MicMusicFormater {
  ...
  constructor(uuid = ELocalMusicDp.dreamlightmic_music_data, defaultValue = {} as TMusic) {
    this.uuid = uuid;
  ...
  parseUnits(step) {
    const result = [];
    // eslint-disable-next-line prefer-const, no-constant-condition
    for (; true; ) {
  ...
  parser(value: string) {
    ...
    if (!length) {
      console.warn(
        ELocalMusicDp.dreamlightmic_music_data,
        'dp data is invalid, cannot be parsed',
        value
      );
    ...
    const opt = Array.from(step(2).value.toString(2).padStart(8, '0')).map(i => Number(i));
    ...
    console.log('dreamlightmic_music_data parser', res);
    return res;
  }
  ...
  formatter(data: TMusic): string {
    ...
    //   data.mode = 2;
    // }
    const { id, v, power, isLight, mode, sensitivity, speed, a, b, c, brightness, colors } = data;
    ...
    if (colors && colors.length) {
      ...
      console.log(ELocalMusicDp.dreamlightmic_music_data, 'formatter', data, res, units, colors);
      return res;
    }
    const res = `${vHex}${powerHex}${idHex}${optNumHex}${speedHex}${sensitivityHex}${aHex}${bHex}${cHex}`;
    console.log(ELocalMusicDp.dreamlightmic_music_data, 'formatter1', data, res);
    return res;
  }
}
