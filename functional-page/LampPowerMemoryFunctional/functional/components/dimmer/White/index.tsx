...
import { useDpSchema, useProps, useSupport, utils } from '@ray-js/panel-sdk';
...
export const White = (props: IProps) => {
  ...
  const { property: { max: brightMax = 1000, min: brightMin = 10 } = {} } =
    schema[dpCodes.bright_value] || {};
    ...
    <View style={style} className={styles.container}>
      {support.isSupportTemp() && (
        <View>
          <View className={styles.textBox}>
            <Text className={styles.label} style={{ color: fontColor }}>
              {Strings.getLang('lpmf_temp')}
            </Text>
            <Text
              className={styles.value}
              style={{ color: themeColor }}
            >{`${temperaturePercent}%`}</Text>
          </View>
          <LampTempSlider
            value={temperature}
      ...
      {support.isSupportBright() && (
        <View>
          <View className={styles.textBox}>
            <Text className={styles.label} style={{ color: fontColor }}>
              {Strings.getLang('lpmf_brightness')}
            </Text>
            <Text
              className={styles.value}
              style={{ color: themeColor }}
            >{`${brightnessPercent}%`}</Text>
          </View>
