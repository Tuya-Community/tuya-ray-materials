...
import { EleRandomTimer } from '@ray-js/electrician-timing-sdk/lib/interface';
...
const AddSchedule: FC = () => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      let res;
      if (+id >= 0) {
        res = await electri.random.update(data, { useDefaultModal: true });
      } else {
        res = await electri.random.add(data, { useDefaultModal: true });
      }
      if (res === 'success') {
        router.back();
      }
      ...
      switch (e.errCode) {
        case 1001: {
          const dpConfig = electri.random.getConfig();
          // 超限
          DialogInstance.alert({
            title: Strings.getLang('ret_tips'),
            message: Strings.formatValue('ret_random_max_tips', dpConfig.max),
            confirmButtonText: Strings.getLang('confirm'),
          });
          return;
        }
        case 1011:
          DialogInstance.alert({
            title: Strings.getLang('ret_tips'),
            message: Strings.getLang('ret_start_than_end'),
            confirmButtonText: Strings.getLang('confirm'),
          });
          return;
        default:
  ...
  }, [weeks, startTime, switches, endTime, timers, saving]);
  ...
  const handleDelete = useCallback(async () => {
    ...
    DialogInstance.confirm({
      ...
      beforeClose: async (action) => {
        if (action === 'confirm') {
          try {
            await removeTimer(+id, timers, electri.random.update, electri.random.remove);
          } catch {
            showToast({
              icon: 'error',
    ...
    if (+id >= 0) {
      if (timers.length > 0) {
        ...
        if (timer) {
          ...
          // 处理开关
          setSwitches([...timer.actions.map((x) => x.code)]);
          return;
        }
      }
    }
