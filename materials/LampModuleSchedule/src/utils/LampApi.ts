...
import { requestCloud, getLaunchOptionsSync } from '@ray-js/ray';
    ...
    requestCloud({
      ...
      success(data) {
        try {
          ...
          console.log(`success1 API 请求失败: %c${url}%o`, errStyle, data);
          resolve(data);
        }
      },
      fail({ errorMsg }) {
        try {
          ...
          console.log(`fail1 try API 请求失败: %c${url}%o`, errStyle, e);
          console.log(`fail1 try API 请求失败: 参数：`, postData);
          reject(e);
        } catch (err) {
          console.log(`fail1 API 请求失败: %c${url}%o，参数：${postData}`, errStyle, errorMsg);
          console.log(`fail1 API 请求失败: 参数：`, postData);
          reject(errorMsg);
        }
      },
    });
...
if (!LampApi.getUiConfig) {
  LampApi.getUiConfig = (productId: any, timestamp = '0') => {
    // tuya.m.app.panel.info.get
    return api(`${getAnonymityNameStr()}.m.app.panel.info.get`, {
      productId,
      timestamp,
    });
  };
...
const handleSyncCloudData = async (syncCloudData: any, syncLocalData: any) => {
  if (!isEmpty(syncCloudData) || !isEmpty(syncLocalData)) {
    if (!isEmpty(syncLocalData)) {
    ...
    StorageUtils.getDevItem(LOCAL_DATA_KEY).then((cacheData: any) => {
      ...
      Object.keys(cacheData).forEach(key => {
        ...
        if (!cloudData) {
          ...
          const { time: cloudTime, data: cloudValue } = cloudData;
