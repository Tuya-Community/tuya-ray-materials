...
class Graphics extends Node {
  ...
  applyConicGradient(...params: any[]) {
    this.cmds.push(['applyConicGradient', arguments]);
    return this;
  }

  renderConicGradient(...params: any[]) {
    this.cmds.push(['renderConicGradient', arguments]);
    return this;
  }
  ...
  strokeGradient(...params: any[]) {
    this.cmds.push(['strokeGradient']);
    return this;
  }
  ...
  render(ctx: CanvasRenderingContext2D) {
    this.cmds.forEach(cmd => {
      ...
      } else if (methodName === 'strokeGradient') {
        ctx.strokeStyle = this.currentGradient;
      } else if (methodName === 'applyConicGradient') {
        ctx.applyConicGradient(ctx, cmd[1][0]);
      } else {
        const result = ctx[methodName].apply(ctx, Array.prototype.slice.call(cmd[1]));
        const gradientList = ['createLinearGradient', 'createRadialGradient'];
        if (gradientList.includes(methodName)) {
