...
const getThumbWidth = (ownerInstance, instanceId, type = 'width') => {
  const thumbStart = queryComponent(ownerInstance, instanceId, '.rayui-percent-slider-thumb-start');
  const thumbStartWidth = thumbStart.getBoundingClientRect()[type];
  if (thumbStartWidth) {
    return thumbStartWidth;
  }
  const thumbEnd = queryComponent(ownerInstance, instanceId, '.rayui-percent-slider-thumb-end');
...
function hideBarThumbs(ownerInstance, instanceId) {
  const thumbs = queryComponentAll(ownerInstance, instanceId, '.rayui-percent-slider-thumb');
  const bar = queryComponent(ownerInstance, instanceId, '.rayui-percent-slider-bar');
...
function updateSlider(ownerInstance, instanceId) {
  ...
  const ost_sliderRange = queryComponent(ownerInstance, instanceId, '.rayui-percent-slider-bar');
...
function setThumb(ownerInstance, instanceId, thumbName, valueInPixels, style, outValue) {
  ...
  const renderThumbs = queryComponentAll(
    ownerInstance,
    instanceId,
    '.rayui-percent-slider-thumb-render'
  );
...
function updateThumbZIndex(ownerInstance, instanceId) {
  ...
  // 左右重叠，右在上
  if (end < mid) {
    const start = queryComponent(ownerInstance, instanceId, '.rayui-percent-slider-thumb-start');
    const end = queryComponent(ownerInstance, instanceId, '.rayui-percent-slider-thumb-end');
...
function setBarStepsWrap(ownerInstance, instanceId) {
  const ost_slider = queryComponent(ownerInstance, instanceId);
  const isVertical = isVerticalMode(ownerInstance, instanceId);
  const wrap = queryComponent(ownerInstance, instanceId, '.rayui-percent-slider-bar-steps_wrap');
