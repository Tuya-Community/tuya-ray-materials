...
const convertValueToOutValue = (ownerInstance, instanceId, value) => {
  ...
  let ret = Number(value / step).toFixed(0) * step;
  return ret + valueMin;
};

const convertOutValueToValue = (ownerInstance, instanceId, outValue) => {
  const valueMin = getValueMin(ownerInstance, instanceId);
  return parseInt(outValue - valueMin);
};
...
const getThumbWidth = (ownerInstance, instanceId, type = 'width') => {
  const thumbStart = queryComponent(ownerInstance, instanceId, '.rayui-touch-slider-thumb-start');
  const thumbStartWidth = thumbStart.getBoundingClientRect()[type];
  if (thumbStartWidth) {
    return thumbStartWidth;
  }
  const thumbEnd = queryComponent(ownerInstance, instanceId, '.rayui-touch-slider-thumb-end');
...
function hideBarThumbs(ownerInstance, instanceId) {
  const thumbs = queryComponentAll(ownerInstance, instanceId, '.rayui-touch-slider-thumb');
  const bar = queryComponent(ownerInstance, instanceId, '.rayui-touch-slider-bar');
...
function updateSlider(ownerInstance, instanceId) {
  ...
  const ost_sliderRange = queryComponent(ownerInstance, instanceId, '.rayui-touch-slider-bar');
...
function setThumb(ownerInstance, instanceId, thumbName, valueInPixels, style, outValue) {
  ...
  const renderThumbs = queryComponentAll(
    ownerInstance,
    instanceId,
    '.rayui-touch-slider-thumb-render'
  );
...
function updateThumbZIndex(ownerInstance, instanceId) {
  ...
  // 左右重叠，右在上
  if (end < mid) {
    const start = queryComponent(ownerInstance, instanceId, '.rayui-touch-slider-thumb-start');
    const end = queryComponent(ownerInstance, instanceId, '.rayui-touch-slider-thumb-end');
...
function setBarStepsWrap(ownerInstance, instanceId) {
  const ost_slider = queryComponent(ownerInstance, instanceId);
  const isVertical = isVerticalMode(ownerInstance, instanceId);
  const wrap = queryComponent(ownerInstance, instanceId, '.rayui-touch-slider-bar-steps_wrap');
...
const onChange = (ownerInstance, instanceId, position, skipDiff = false) => {
  ...
  // publish
  ownerInstance.triggerEvent('onChange', eventData);
...
const onEnd = (event, ownerInstance, instanceId) => {
  ...
  // publish
  ownerInstance.triggerEvent('onEnd', eventData);
...
const onStart = (event, ownerInstance, instanceId) => {
  ...
  // publish
  ownerInstance.triggerEvent('onStart', eventData);
...
const getMathTypeFunc = mathtype => {
  if (mathtype in Math) {
...
function calcText(value, scale, type) {
  const func = getMathTypeFunc(type);
  if (func) {
    return func(Number(value) * scale);
  }
  return value;
}
