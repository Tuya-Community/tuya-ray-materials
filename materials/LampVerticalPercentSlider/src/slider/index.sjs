...
const createPropObserver = prop => instanceId =>
  function (newValue, oldValue, ownerInstance) {
    ...
    if (prop === 'end') {
      setEnd(ownerInstance, instanceId, newValue, 'createPropObserver');
    }
...
const setEnd = (ownerInstance, instanceId, value, tag) => {
...
const getThumbWidth = (ownerInstance, instanceId, type = 'width') => {
  const thumbStart = queryComponent(
    ownerInstance,
    instanceId,
    '.rayui-vertical-percent-slider-thumb-start'
  );
  ...
  const thumbEnd = queryComponent(
    ownerInstance,
    instanceId,
    '.rayui-vertical-percent-slider-thumb-end'
  );
...
function hideBarThumbs(ownerInstance, instanceId) {
  const thumbs = queryComponentAll(
    ownerInstance,
    instanceId,
    '.rayui-vertical-percent-slider-thumb'
  );
  const bar = queryComponent(ownerInstance, instanceId, '.rayui-vertical-percent-slider-bar');
...
function updateSlider(ownerInstance, instanceId) {
  ...
  const ost_sliderRange = queryComponent(
    ownerInstance,
    instanceId,
    '.rayui-vertical-percent-slider-bar'
  );
  ...
  if (ost_slider && ost_sliderRange) {
    ...
    const endValue = isRerverse
      ...
      : _endValueInPixels;
    setOstValue(ownerInstance, instanceId, '_endValueInPixels', endValue);
    ...
    if (showSteps) {
      const steps = queryComponent(
        ownerInstance,
        instanceId,
        '.rayui-vertical-percent-slider-steps'
      );
      if (steps) {
        steps.setStyle({
...
function setThumb(ownerInstance, instanceId, thumbName, valueInPixels, style) {
  ...
  const renderThumbs = queryComponentAll(
    ownerInstance,
    instanceId,
    '.rayui-vertical-percent-slider-thumb-render'
  );
...
function setBarStepsWrap(ownerInstance, instanceId) {
  ...
  const wrap = queryComponent(
    ownerInstance,
    instanceId,
    '.rayui-vertical-percent-slider-bar-steps_wrap'
  );
...
const handleTrackMouseDown = instanceId => (event, ownerInstance) => {
  ...
  if (!isRange && !hideThumb && isEnableTouch(ownerInstance, instanceId) && !enableTouchBar) {
    ...
    if (thumbEnd) {
      ...
      setEnd(ownerInstance, instanceId, moveValue, 'handleTrackMouseDown');
  ...
  if (ost.isMoving) {
    ...
    switch (ost.currentThumbName) {
      ...
      case 'end':
        ...
        setEnd(ownerInstance, instanceId, moveValue, 'onMouseMove');
        onChange(ownerInstance, instanceId, 'end');
        break;
    }
    ...
    setOstValue(ownerInstance, instanceId, '_start', 0);
    setEnd(ownerInstance, instanceId, moveValue, 'onMouseMove2');
...
const onEnd = (event, ownerInstance, instanceId, from = '') => {
  const target = event.instance;
  const isRerverse = isReverseMode(ownerInstance, instanceId);
  const reverseRange = getMax(ownerInstance, instanceId) - getRangeValue(ownerInstance, instanceId);

  const nextStart = getStart(ownerInstance, instanceId);
  let nextEnd = isRerverse ? reverseRange : getEnd(ownerInstance, instanceId);
...
const onStart = (event, ownerInstance, instanceId) => {
  ...
  const rangeValue = getRangeValue(ownerInstance, instanceId);
  const maxValue = getMax(ownerInstance, instanceId);
  const reverseRange = maxValue - rangeValue;

  const nextStart = getStart(ownerInstance, instanceId);
  const nextEnd = isRerverse ? reverseRange : getEnd(ownerInstance, instanceId);
  const nextRange = isRerverse ? reverseRange : rangeValue;
...
const onMouseUp = instanceId => (event, ownerInstance) => {
  ...
  onEnd(event, ownerInstance, instanceId, 'onMouseUp');
...
const onTrackMouseUp = instanceId => (event, ownerInstance) => {
  ...
  onEnd(event, ownerInstance, instanceId, 'onTrackMouseUp');
  setOstValue(ownerInstance, instanceId, 'isMoving', false);
